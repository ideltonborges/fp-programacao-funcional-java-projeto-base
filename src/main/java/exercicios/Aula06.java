...
public class Aula06 extends Aula {
     ...
     */
    private final Predicate<Estudante> mulheresAprovadas = ((Predicate<Estudante>) Estudante::isMulher).and(Estudante::hasCurso).and(Estudante::isAprovado); //TODO: Atribua aqui o predicado composto com o filtro indicado acima
    ...
    public List<Estudante> getEstudantesMulheresAprovadasOrdenadasPorCursoAndNota() {
        Comparator<Estudante> compEstudante = Comparator.comparing(Estudante::getCurso).thenComparingDouble(Estudante::getNota);
        return estudantes.stream().filter(mulheresAprovadas).sorted(compEstudante).toList();
    }
    ...
    public List<Estudante> getEstudantesMulheresAprovadasOrdenadasPorCursoDecrescenteAndNotaCrescente() {
        var comp = Comparator.comparing(Estudante::getCurso).reversed().thenComparingDouble(Estudante::getNota);
        return estudantes.stream().filter(mulheresAprovadas).sorted(comp).toList();
    }
    ...
    public List<Estudante> getEstudantesMulheresAprovadasNaoOrdenadasModificavel() {
       return estudantes.stream().filter(mulheresAprovadas).collect(Collectors.toList());
    }
    ...
    public List<Estudante> getEstudantesMulheresAprovadasOrdenadasTotalmenteDecrescente() {
        var comp = Comparator.comparing(Estudante::getCurso).thenComparingDouble(Estudante::getNota).reversed();
        return estudantes.stream().filter(mulheresAprovadas).sorted(comp).toList();
    }
    ...
    public List<Estudante> getEstudantesMulheresAprovadasOrdenadasPorCursoCrescenteAndNotaDecrescente() {
        var comp = Comparator.comparing(Estudante::getCurso).thenComparing(Comparator.comparingDouble(Estudante::getNota).reversed());
        return estudantes.stream().filter(mulheresAprovadas).sorted(comp).toList();
    }
}
